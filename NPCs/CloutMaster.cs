...
{
    [AutoloadHead]
    public class CloutMaster : ModNPC
    {
        ...
        {
            button = "Buy Lit Tings";   //this defines the buy button name
        }
        ...
        public override void SetupShop(Chest shop, ref int nextSlot)       //Allows you to add items to this town NPC's shop. Add an item by setting the defaults of shop.item[nextSlot] then incrementing nextSlot.
        {
            ...
            if (NPC.downedBoss3)   //this make so when Skeletron is killed the town npc will sell this
            {
                ...
                nextSlot++;
                shop.item[nextSlot].SetDefaults(mod.ItemType("LiquidClout"));
                nextSlot++;
                shop.item[nextSlot].SetDefaults(mod.ItemType("CloutHoodie"));
                nextSlot++;
                shop.item[nextSlot].SetDefaults(mod.ItemType("CloutPants"));
                nextSlot++;
        ...
        { 
            int OldFriendNPC = mod.NPCType("OldFriendNPC");   //this make so when this npc is close to Wizard
            if (OldFriendNPC >= 0 && Main.rand.Next(4) == 0)    //has 1 in 3 chance to show this message
            {
                return "Tell " + Main.npc[OldFriendNPC].GivenName + " that he's wasting his time here.";
            }
            if (OldFriendNPC >= 0 && Main.rand.Next(4) == 0)    //has 1 in 3 chance to show this message
            {
                return "" + Main.npc[OldFriendNPC].GivenName + " Is crazy, don't listen to what he says.";
            }
                switch (Main.rand.Next(4))    //this are the messages when you talk to the npc
            {
                case 0:
                    return "GANG";
                case 1:
                    return "I ran away from home, no reason. Just did.";
                case 2:
                    return "Fanta cost more than your copper money!";
                case 3:
                    return "Clout Man's new album dropping TOMMOROW!";
                case 4:
                    return "My real name is Leon, but im a CloutMaster, so people call me Clout Man.";
                default:
                    return "Friendly Gang!";
            }
          }
        ...
        {
            scale = 1f;
            item = Main.itemTexture[mod.ItemType("CloutMansSword")]; //this defines the item that this npc will use
            itemSize = 64;
        }
